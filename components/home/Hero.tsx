...
export default function Hero() {
  ...
  return (
    <section
      className="relative w-full min-h-[660px] sm:min-h-[700px] flex items-center overflow-hidden bg-altiusNavy pt-24 pb-14 sm:pt-28 sm:pb-16"
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
      aria-label="Altius Education Programs Carousel"
    >
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 w-full text-white">
        <div className="max-w-2xl space-y-6">
          ...
          {/* CSS Grid Stack Container: All slides share the exact same grid cell so height never changes */}
          <div className="grid grid-cols-1 grid-rows-1">
            {HERO_SLIDES.map((slide, index) => {
              ...
              return (
                <div
                  key={slide.id}
                  className={`col-start-1 row-start-1 transition-all duration-500 ease-out space-y-4 sm:space-y-5 flex flex-col justify-between ${
                    isActive
                      ? 'opacity-100 translate-x-0 pointer-events-auto z-10'
                      : 'opacity-0 translate-x-6 pointer-events-none z-0 select-none'
                  }`}
                >
                  {/* Program Badge */}
                  <div>
          ...
          {/* Student Proof Badges placed in place of the slide numbers */}
          <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-white/15">
            <div className="flex items-center gap-3.5">
              <div className="flex -space-x-2.5 overflow-hidden">
                <div className="relative inline-block h-8 w-8 rounded-full ring-2 ring-altiusNavy overflow-hidden bg-gray-200">
