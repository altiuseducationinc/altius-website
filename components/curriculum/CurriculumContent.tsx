...
//       {/* Hero Banner Section matching Programs / Reference Image Style */}
//       <section className="relative w-full">
//         <div className="relative h-[320px] sm:h-[400px] w-full bg-altiusNavy">
//           <Image
//             src="/CurriculumHero.png"
//             alt="Altius Education Curriculum & Resources"
//             fill
//             priority
//             className="object-cover object-center opacity-30 mix-blend-multiply select-none pointer-events-none"
//           />
//           {/* Rich Color Tint Gradient Overlay matching reference image */}
//           <div className="absolute inset-0 bg-gradient-to-tr from-altiusNavy/95 via-altiusNavy/70 to-altiusBlue/40 flex items-center justify-center text-center px-6">
//             <div className="max-w-4xl mx-auto space-y-4 pt-12">
//               <span className="bg-gradient-to-r from-amber-400 via-altiusGold to-yellow-300 bg-clip-text text-transparent font-bold uppercase tracking-[0.2em] text-[11px] sm:text-xs">
...
//               </h1>
//               <p className="text-gray-200 text-xs sm:text-sm lg:text-base leading-relaxed max-w-2xl mx-auto font-light">
...
//         {/* Official Ministry Redirection Notice Banner */}
...
export default function CurriculumContent() {
  ...
  return (
    <div className="bg-white min-h-screen text-altiusNavy pb-24 font-sans">
      {/* Render the separated Hero Component */}
      <CurriculumHero />

      {/* Main Container */}
      <div className="max-w-7xl mx-auto px-6 pt-10 space-y-6">
        {/* Official Ministry Redirection Notice Banner */}
        <div className="bg-gradient-to-r from-amber-500/10 via-amber-400/5 to-transparent border-l-4 border-altiusGold bg-white rounded-[8px] p-4 sm:p-5 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4 border border-amber-200/60">
          <div className="flex items-start sm:items-center gap-3.5">
            <div className="w-9 h-9 rounded-[8px] bg-altiusGold/20 text-altiusNavy flex items-center justify-center shrink-0 font-bold">
              <svg className="w-5 h-5 text-amber-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 bg-altiusNavy hover:bg-altiusNavy/90 text-altiusGold font-bold text-xs px-4 py-2.5 rounded-[8px] transition shadow-xs whitespace-nowrap self-start md:self-auto shrink-0"
          >
            <span>Visit Ontario DCP Directory</span>
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
        ...
        {/* Toolbar: Category Tabs + Search Bar (Optimized for Mobile Wrapping) */}
        <div className="bg-white p-4 sm:p-5 rounded-[8px] border border-gray-200 shadow-xs space-y-4">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            
            {/* Category Toggle Tabs - Stacks gracefully on mobile */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-1.5 bg-slate-100/90 rounded-[8px] border border-gray-200/80 w-full lg:w-auto">
              <button
                type="button"
                onClick={() => setActiveCategory('elementary')}
                className={`py-2.5 px-4 sm:px-5 text-xs font-bold uppercase tracking-wider rounded-[6px] transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer ${
                  activeCategory === 'elementary'
                    ? 'bg-gradient-to-r from-blue-950 via-altiusNavy to-blue-900 text-altiusGold shadow-sm'
                    : 'text-gray-600 hover:text-altiusNavy hover:bg-white'
                }`}
            ...
            <div className="relative w-full lg:w-80 xl:w-96">
              <input
                ...
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search subject, grade, year..."
                className="w-full pl-10 pr-9 py-2.5 bg-slate-50 border border-gray-300 rounded-[8px] text-xs sm:text-sm text-gray-800 placeholder-gray-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-altiusBlue/30 focus:border-altiusBlue transition shadow-2xs"
              />
              <svg
                className="w-4 h-4 text-gray-400 absolute left-3.5 top-3"
                fill="none"
        ...
        {filteredSubjects.length === 0 ? (
          <div className="bg-white rounded-[8px] p-12 text-center border border-gray-200 space-y-4 shadow-sm">
            <div className="w-14 h-14 bg-gray-100 text-gray-400 rounded-full flex items-center justify-center mx-auto">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
            {filteredSubjects.map((subject) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="group bg-white rounded-[8px] border border-gray-200/90 shadow-2xs hover:shadow-xl hover:border-altiusGold/60 hover:-translate-y-1 transition-all duration-300 p-5 flex flex-col justify-between relative overflow-hidden"
              >
                <div
                  style={{ backgroundColor: subject.color }}
                  className="absolute top-0 left-0 right-0 h-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div
                      style={{ backgroundColor: subject.color }}
                      className="w-12 h-12 rounded-[8px] flex items-center justify-center text-white shadow-sm shrink-0 select-none group-hover:scale-105 transition-transform duration-300"
                    >
                      {renderSubjectIcon(subject.iconType)}
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 text-gray-400 group-hover:text-altiusBlue transition-colors">
                      ...
                      <div className="w-8 h-8 rounded-[8px] bg-slate-50 border border-gray-200 flex items-center justify-center group-hover:bg-altiusNavy group-hover:text-altiusGold group-hover:border-altiusNavy transition-all duration-200">
                        <svg
                          className="w-4 h-4 transform group-hover:translate-x-0.5 transition-transform"
                          fill="none"
                          stroke="currentColor"
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="inline-block bg-slate-100 text-gray-600 px-2.5 py-0.5 rounded-[6px] text-[11px] font-medium">
                      {subject.grades}
                    </div>
        ...
        {/* Academic Alignment CTA Card */}
        <div className="bg-gradient-to-r from-blue-950 via-altiusNavy to-blue-900 text-white rounded-[8px] p-8 sm:p-12 shadow-xl relative overflow-hidden mt-12">
          <div className="absolute right-0 top-0 w-96 h-96 bg-altiusGold/10 rounded-full blur-3xl pointer-events-none" />
          ...
          <div className="relative z-10 max-w-3xl space-y-5">
            ...
            <div className="flex flex-wrap gap-4 pt-2">
              <Link
                href="/programs"
                className="bg-gradient-to-r from-amber-500 via-altiusGold to-yellow-400 text-altiusNavy text-xs font-extrabold px-6 py-3.5 rounded-[8px] shadow-lg hover:from-amber-400 hover:to-yellow-300 transition uppercase tracking-wider inline-flex items-center gap-2"
              >
                <span>Explore Altius Programs</span>
                <span>&rarr;</span>
              </Link>
              <Link
                href="/contact"
                className="border-2 border-white/80 text-white text-xs font-bold px-6 py-3.5 rounded-[8px] hover:bg-white hover:text-altiusNavy transition uppercase tracking-wider backdrop-blur-xs"
              >
                Book An Academic Consultation
              </Link>
            </div>
